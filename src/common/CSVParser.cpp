...
bool CSVParser::ParseTaxonomyCSV(const wxString& content, std::map<long, TaxonomyEntry>& taxonomyMap)
{
    ...
    while (tokenizer.HasMoreTokens())
    {
        ...
        // Parse parent ID from field 3 (parentNameUsageID)
        // Extract ID from URL like "https://www.inaturalist.org/taxa/48460"
        wxString parentUrl = fields[3];
        if (!parentUrl.IsEmpty())
        {
            int lastSlash = parentUrl.Find('/', true);
            if (lastSlash != wxNOT_FOUND)
            {
                wxString parentIdStr = parentUrl.Mid(lastSlash + 1);
                long parentId;
                if (parentIdStr.ToLong(&parentId))
        ...
        // CSV fields: id, taxonID, identifier, parentNameUsageID, kingdom, phylum, class, order, family, genus, specificEpithet, infraspecificEpithet, modified, scientificName, taxonRank, references
        entry.kingdom = fields[4];
        entry.phylum = fields[5];
        entry.taxonClass = fields[6];
        entry.order = fields[7];
        entry.family = fields[8];
        if (fields.size() > 9)
            entry.genus = fields[9];
        if (fields.size() > 10)
            entry.specificEpithet = fields[10];
        if (fields.size() > 11)
            entry.infraspecificEpithet = fields[11];
        // Field 12 is modified date
        if (fields.size() > 13)
            entry.scientificName = fields[13];
        if (fields.size() > 14)
            entry.taxonRank = fields[14];
